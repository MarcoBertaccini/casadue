...
async function checkAndGenerateRecurring() {
  ...
  // Fetch all expenses this month — both system-generated and manual
  const { data: existing } = await db.from('expenses')
    .select('recurring_id, description').gte('date', from).lte('date', to);

  // A recurring template is already "covered" if there's an expense this month
  // with matching recurring_id (generated by system) OR matching description (added manually)
  ...
  for (const r of recurrings) {
    ...
    if (existingDescriptions.has(r.description.toLowerCase().trim())) continue;
